...
| 7 | `节点组边策略` | 所有 node_groups.edge_strategies（一行一策略） | 恒定 |
| 8 | `节点告警` | 所有物理节点告警 | 仅拓扑绑了 alarm_schema 时 |
| 9 | `节点组告警` | 所有组告警模板 | 仅拓扑绑了 alarm_schema 时 |
...
- **A1 单元格 comment 存原始 code/marker**（导入时按 code 精确匹配，不受 Sheet 名清洗/用户重命名影响）：
  ...
  - 节点组边策略 Sheet: `__NODE_GROUP_EDGE_STRATEGY__=1`
...
**只列 6 类数据 Sheet**（不列 `_使用说明` 和 `拓扑元信息`——它们位置固定、易找）：

| 类别 | 列 |
|---|---|
| 类别 | `节点 / 边 / 节点组 / 节点组边策略 / 节点告警 / 节点组告警` |
| 类型代码 | nodeType.code / edgeType.code（其它行留空） |
...
一行一条边策略（跨所有节点组，方便一眼看清整个拓扑里节点组之间的连接关系 + 批量改）：

| # | 列 | 类型 | 说明 |
|---|---|---|---|
| 1 | 源组名 | 字符串 | `node_groups.group_name` |
| 2 | 目标 | 字符串 | 目标组名 **或** 普通节点名（hybrid 模式引用节点） |
| 3 | 目标类型 | 枚举 | `组 / 节点` |
| 4 | 边类型代码 | 字符串 | `edge_types.code` |
| 5 | 模式 | 枚举 | `modulo / one_to_n / all_to_all / dense` |
| 6 | K | 数字 | 仅 modulo / one_to_n 用，其它模式留空 |

**"目标类型" 列的必要性：** 现有 `edge_strategies.target_group_id` 可以是节点组 id 也可以是普通节点 id（hybrid 模式）——这个列区分两种引用类型；不加的话导入时组和节点重名会歧义。

**A1 comment：** `__NODE_GROUP_EDGE_STRATEGY__=1`

**导入行为：** 组装成 `edge_strategies` JSON，塞进对应 `node_groups.edge_strategies` 字段。校验：
- 源组名找不到 → 行级错误
- 目标（组名/节点名）在同 workbook 里找不到 → 行级错误
- 模式不合法 → 行级错误
- 模式为 modulo / one_to_n 但 K 为空 → 行级错误
...
7. 单事务里：`INSERT topology` → 所有 nodes + node_attrs + canvas_nodes → 所有 edges + edge_attrs → node_groups（此时 edge_strategies 先留 NULL）→ 解析 `节点组边策略` Sheet 组装成各组的 edge_strategies JSON → `UPDATE node_groups SET edge_strategies` → node_alarms + attrs → node_group_alarms + attrs
...
2. `format_edge_strategy_row` / `parse_edge_strategy_row` 循环一致性——4 种模式 + 目标类型（组 / 节点）
3. Sheet 名清洗 + dedupe 后缀
4. A1 comment 存/取 `__NODE_TYPE_CODE__` / `__EDGE_TYPE_CODE__` / `__NODE_GROUP__` / `__NODE_GROUP_EDGE_STRATEGY__` 等
...
16. **`_总表` hyperlink 指向的目标 Sheet 存在**（openpyxl 读 hyperlink 属性验证；总表列 6 类）
17. **materialize 组的物理节点：** 一组材化后有 3 物理节点，导出时物理节点走 nodes Sheet + 组定义走 节点组 Sheet + 关联通过 `所属组` 列
18. **节点组边策略 round-trip：** 造 2 组 + 组间 all_to_all 边策略 + hybrid 边策略指向普通节点 → 导出 → 导入 → `edge_strategies` JSON 保真
19. **节点组边策略源组未找到：** `节点组边策略` Sheet 里源组名 'ghost' 在 `节点组` Sheet 不存在 → 行级错误
20. **节点组边策略目标歧义：** hybrid 模式但"目标类型"列填错（应为节点却写组）→ 行级错误
